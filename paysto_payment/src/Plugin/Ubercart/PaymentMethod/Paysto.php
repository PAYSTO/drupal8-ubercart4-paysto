...
class Paysto extends PaymentMethodPluginBase implements OffsitePaymentMethodPluginInterface {
	 ...
	 */
	protected $url = 'https://api.paysto.eu/api/checkout/redirect/';
	/**
	 * @var string
	 */
	...
	public function getDisplayLabel( $label ) {
		...
		$build['image'] = [
			...
			'#alt'        => $this->t( 'paysto' ),
			'#attributes' => array( 'class' => array( 'uc-paysto-logo' ) )
		];
	...
	public function buildConfigurationForm( array $form, FormStateInterface $form_state ) {
		...
		$form['language']    = array(
			...
			'#description'   => $this->t( 'Adjust language on paysto pages.' ),
			'#options'       => array(
				'en' => $this->t( 'English' ),
				'ru' => $this->t( 'Russian' ),
				'ua' => $this->t( 'Ukranian' ),
	...
	public function buildRedirectForm( array $form, FormStateInterface $form_state, OrderInterface $order = null ) {
		...
		$dataSettings = [
			...
			'merchant_id'         => $paystoSettings['merchant_id'],
			'order_desc'          => $this->t( 'Order Pay #' ) . $order->id(),
			'amount'              => $amount,
			'preauth'             => $paystoSettings['preauth'] ? 'Y' : 'N',
			'currency'            => $paystoSettings['currency'] != '' ? $paystoSettings['currency'] : $order->getCurrency(),
			'server_callback_url' => $paystoSettings['back_url'] == '' ? Url::fromRoute( 'uc_paysto.notification', [], [ 'absolute' => true ] )->toString() : $paystoSettings['back_url'],
		...
		$dataSettings['signature'] = self::getSignature( $dataSettings, $paystoSettings['secret_key'] );
