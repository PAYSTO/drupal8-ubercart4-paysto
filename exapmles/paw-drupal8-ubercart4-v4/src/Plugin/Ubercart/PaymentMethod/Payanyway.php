...
namespace Drupal\uc_payanyway\Plugin\Ubercart\PaymentMethod;
...
/**
 * Defines the payanyway payment method.
 *
 * @UbercartPaymentMethod(
 *   id = "payanyway",
 *   name = @Translation("PayAnyWay"),
 *   redirect = "\Drupal\uc_payanyway\Form\PayanywayForm",
 * )
 */
class Payanyway extends PaymentMethodPluginBase implements OffsitePaymentMethodPluginInterface {

  /**
   * {@inheritdoc}
   */
  public function getDisplayLabel($label) {
    $build['#attached']['library'][] = 'uc_payanyway/payanyway.styles';
    $build['label'] = array(
      '#plain_text' => $label,
      '#suffix' => '<br />',
    );
    $build['image'] = array(
      '#theme' => 'image',
      '#uri' => drupal_get_path('module', 'uc_payanyway') . '/images/2co_logo.jpg',
      '#alt' => $this->t('payanyway'),
      '#attributes' => array('class' => array('uc-payanyway-logo')),
    );
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    $form['sid'] = array(
      ...
      '#description' => $this->t('Your payanyway vendor account number.'),
    ...
    $form['secret_word'] = array(
      ...
      '#description' => $this->t('The secret word entered in your payanyway account Look and Feel settings.'),
    ...
    $form['language'] = array(
      ...
      '#description' => $this->t('Adjust language on payanyway pages.'),
      '#options' => array(
        'en' => $this->t('English'),
        'sp' => $this->t('Spanish'),
      ),
    ...
    $form['notification_url'] = array(
      ...
      '#description' => $this->t('Pass this URL to the <a href=":help_url">instant notification settings</a> parameter in your payanyway account. This way, any refunds or failed fraud reviews will automatically cancel the Ubercart order.', [':help_url' => Url::fromUri('https://www.payanyway.ru/static/va/documentation/INS/index.html')->toString()]),
      '#default_value' => Url::fromRoute('uc_payanyway.notification', [], ['absolute' => TRUE])->toString(),
  ...
  public function buildRedirectForm(array $form, FormStateInterface $form_state, OrderInterface $order = NULL) {
    ...
    $data = array(
      ...
      'pay_method' => 'CC',
      'x_receipt_link_url' => Url::fromRoute('uc_payanyway.ruplete', ['cart_id' => \Drupal::service('uc_cart.manager')->get()->getId()], ['absolute' => TRUE])->toString(),
    ...
    if ('direct' == $this->configuration['checkout_type']) {
      $form['#attached']['library'][] = 'uc_payanyway/payanyway.direct';
    }

    $host = $this->configuration['demo'] ? 'sandbox' : 'www';
    $form['#action'] = "https://$host.payanyway.ru/checkout/purchase";
