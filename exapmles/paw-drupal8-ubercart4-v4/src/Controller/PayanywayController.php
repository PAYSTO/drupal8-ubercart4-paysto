...
namespace Drupal\uc_payanyway\Controller;
...
/**
 * Controller routines for uc_payanyway.
 */
class PayanywayController extends ControllerBase {
  ...
  /**
   * Constructs a PayanywayController.
   *
  ...
  /**
   * Finalizes payanyway transaction.
   *
   * @param int $cart_id
   *   The cart identifier.
  ...
  public function complete($cart_id = 0, Request $request) {
    \Drupal::logger('uc_payanyway')->notice('Receiving new order notification for order @order_id.', ['@order_id' => SafeMarkup::checkPlain($request->request->get('merchant_order_id'))]);
    ...
    if ($plugin->getPluginId() != 'payanyway') {
    ...
    if (Unicode::strtolower($key) != Unicode::strtolower($valid)) {
      uc_order_comment_save($order->id(), 0, $this->t('Attempted unverified payanyway completion for this order.'), 'admin');
      throw new AccessDeniedHttpException();
    }

    if ($request->request->get('demo') == 'Y' xor $configuration['demo']) {
      \Drupal::logger('uc_payanyway')->error('The payanyway payment for order <a href=":order_url">@order_id</a> demo flag was set to %flag, but the module is set to %mode mode.', array(
    ...
    if ($request->request->get('credit_card_processes') == 'Y' && is_numeric($request->request->get('total'))) {
      $comment = $this->t('Paid by @type, payanyway.ru order #@order.', ['@type' => $request->request->get('pay_method') == 'CC' ? $this->t('credit card') : $this->t('echeck'), '@order' => SafeMarkup::checkPlain($request->request->get('order_number'))]);
      uc_payment_enter($order->id(), 'payanyway', $request->request->get('total'), 0, NULL, $comment);
    }
    else {
      drupal_set_message($this->t('Your order will be processed as soon as your payment clears at payanyway.ru.'));
      uc_order_comment_save($order->id(), 0, $this->t('@type payment is pending approval at payanyway.ru.', ['@type' => $request->request->get('pay_method') == 'CC' ? $this->t('Credit card') : $this->t('eCheck')]), 'admin');
    }
  ...
  /**
   * React on INS messages from payanyway.
   *
   * @param \Symfony\Component\HttpFoundation\Request $request
   *   The request of the page.
   */
  public function notification(Request $request) {
    ...
    \Drupal::logger('uc_payanyway')->notice('Received payanyway notification with following data: @data', ['@data' => print_r($values->all(), TRUE)]);
    ...
    if ($values->has('message_type') && $values->has('md5_hash') && $values->has('message_id')) {
      ...
      $payanyway_order_id = $values->get('sale_id');
      $payanyway_invoice_id = $values->get('invoice_id');
      $hash = strtoupper(md5($payanyway_order_id . $sid . $payanyway_invoice_id . $secret_word));

      if ($hash != $values->get('md5_hash')) {
        \Drupal::logger('uc_payanyway')->notice('payanyway notification #@num had a wrong hash.', ['@num' => $values->get('message_id')]);
        die('Hash Incorrect');
      }
...
// payment was successful, like PayPal IPN does ...
          ...
          case 'fail':
            ...
            uc_order_comment_save($order_id, 0, $this->t('Order have not passed payanyway fraud review.'));
            die('fraud');
            break;
        }
      }
      elseif ($values->get('message_type') == 'REFUND_ISSUED') {
        // @todo uc_order_update_status($order_id, uc_order_state_default('canceled'));
        $order->setStatusId('canceled')->save();
        uc_order_comment_save($order_id, 0, $this->t('Order have been refunded through payanyway.'));
        die('refund');
      }
    }
    die('ok');
